...
async function top3blog() {
    var url = 'http://localhost:8080/api/public/get-top-3-blog';
    const response = await fetch(url, {
        method: 'GET',
        headers: new Headers({
        })
    ...
    for (i = 0; i < list.length; i++) {
        ...
        <div class="singleblg">
            <a href="ctblog?id=${list[i].id}"><img src="${list[i].imageBanner}" class="imgblogindex"></a>
            <p class="titleblogindex"><a href="ctblog?id=${list[i].id}">${list[i].title}</a></p>
