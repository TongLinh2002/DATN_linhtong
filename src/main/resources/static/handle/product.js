...
async function loadProductByCategory(page) {
    ...
    for (i = 0; i < list.length; i++) {
        var star = '';
        if(list[i].star != null && list[i].star > 0){
        ...
        main += `<div class="images_1_of_4 products-info col-md-3">
                    <div class="bdr">
                    <a href="chitietsp?id=${list[i].id}">
                    <div class="product-img"><img src="${list[i].imageBanner}"></div>
                     <div class="divnamepro">
                        <h3 class="namepro"><span>${list[i].name}</span></h3>
...
async function chitietsp() {
    ...
    document.getElementById("giaspcu").innerHTML = pro.oldPrice == null?'':(formatmoney(pro.oldPrice))
    document.getElementById("hangsx").innerHTML = pro.tradeMark.name
    document.getElementById("soluongcon").innerHTML = pro.quantity +" sản phẩm"
...
async function loadDmIndexLeft() {
    var url = 'http://localhost:8080/api/public/allcategory';
    const response = await fetch(url, {
        method: 'GET',
        headers: new Headers({
    ...
    for(i=0; i< list.length; i++){
        // main += ' <li><a style="text-decoration: none" href="listsp?category='+list[i].id+'"><img alt="" src="image/iconmenu.png" style="color: red"> <span style=" font-size: 15px; font-weight: 900;margin-left:2px"><span>'+list[i].name+'</span></span></a></li>'
        main += `<a href="listsp?category=${list[i].id}" class="category-item"><i class="fa fa-caret-right"></i> ${list[i].name}</a>`
    }
